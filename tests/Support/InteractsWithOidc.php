...
namespace SocialiteProviders\OpenIDConnect\Tests\Support;
...
trait InteractsWithOidc
{
    ...
    protected function lastRequestTo(string $path): ?\Psr\Http\Message\RequestInterface
    {
        foreach (array_reverse($this->httpHistory) as $entry) {
            if ($entry['request']->getUri()->getPath() === $path) {
